...
def training(args):
    ...
    if True:
        ...
        for i in range(10):
            ...
            text_prompt = 'head'#, black in left, black in right, bladder in upper middle, rectum, bone, tumor, hole, vessel, fat, high signal, low signal, muscle'
            ...
            for idx in range(min_len):#(min_len-20, min_len+20, 2):
                ...
                with torch.no_grad():
                    ...
                    tgt_pred_msk, tgt_boxes, tgt_phrases, tgt_logits, tgt_emb  = model.predict(image_pil=tgt_input,
                      text_prompt=text_prompt)
                    if len(src_boxes)==0 or len(tgt_boxes)==0:
                        continue
                    for bbx in src_boxes:
                        checking1 = check_overlap(bbx, src_seg[idx].squeeze())
                        if checking1:
                            break
                    for bbx in tgt_boxes:
                        checking2 = check_overlap(bbx, tgt_seg[idx].squeeze())
                        if checking2:
                            break
